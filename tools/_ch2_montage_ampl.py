...
names=['image35','image96','image97']
try: font=ImageFont.truetype('arial.ttf',18)
except Exception: font=ImageFont.load_default()
imgs=[]
for n in names:
    try: im=Image.open(base/(n+'.png')).convert('RGB')
    except Exception: im=Image.new('RGB',(200,40),'white'); ImageDraw.Draw(im).text((4,10),n+' (no png)',fill='red')
    s=max(1.0,110/im.height)
...
out=pathlib.Path('build/ch2_ampl.png'); c.save(out); print('saved',out,c.size)
