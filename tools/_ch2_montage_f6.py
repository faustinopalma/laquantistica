...
names = ['image8','image9','image14','image15','image18']
try: font = ImageFont.truetype('arial.ttf', 20)
except Exception: font = ImageFont.load_default()
imgs=[]
for n in names:
    im=Image.open(base/(n+'.png')).convert('RGB')
    s=min(1.0,760/im.width)
    if im.height*s<50: s=50/im.height
    im=im.resize((max(1,int(im.width*s)),max(1,int(im.height*s))))
    imgs.append((n,im))
pad=10; lh=28
W=max(800,max(im.width for _,im in imgs)+2*pad)
H=sum(im.height+lh+pad for _,im in imgs)+pad
...
out=pathlib.Path('build/ch2_fig6to10.png'); c.save(out); print('saved',out,c.size)
