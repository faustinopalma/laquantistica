...
function Invoke-Pass {
  param([string[]]$Domains, [int]$Throttle)
  $Domains | ForEach-Object -Parallel {
    $d = $_
    $uri = "https://rdap.org/domain/$d"
    try {
      $r = Invoke-WebRequest -Uri $uri -Method Get -TimeoutSec 20 -MaximumRedirection 5 -ErrorAction Stop
...
Write-Output "checking $($all.Count) domains"

$final = @{}
$pending = [System.Collections.Generic.List[string]]::new()
$all | ForEach-Object { $pending.Add($_) }

for ($p = 1; $p -le $Passes -and $pending.Count -gt 0; $p++) {
  $thr = if ($p -eq 1) { $Throttle } else { 2 }
  Write-Output "pass $p : $($pending.Count) domains (throttle $thr)"
  $res = Invoke-Pass -Domains $pending.ToArray() -Throttle $thr
  $next = [System.Collections.Generic.List[string]]::new()
  foreach ($r in $res) {
    if ($r.State -eq 'RETRY') { $next.Add($r.Domain) }
    else { $final[$r.Domain] = $r }
  }
  $pending = $next
}
