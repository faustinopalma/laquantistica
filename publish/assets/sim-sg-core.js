...
var ACC = '#7b2d26', BLU = '#1a5276', INK = '#1b1e23', DIM = '#c9c4b6';
...
function sgMagnet(cx, cy, angle, label, parent) {
  ...
  if (a > 0.5 && a < 359.5) {
    el('line', { x1: cx, y1: cy - 52, x2: cx, y2: cy - 16, stroke: '#9a978c', 'stroke-width': 1, 'stroke-dasharray': '5 3 1 3' }, parent);
    var R = 30, rad = a * Math.PI / 180, x2 = cx + R * Math.sin(rad), y2 = cy - R * Math.cos(rad), sweep = a <= 180 ? 1 : 0;
    el('path', { d: 'M ' + cx + ' ' + (cy - R) + ' A ' + R + ' ' + R + ' 0 0 ' + sweep + ' ' + x2.toFixed(1) + ' ' + y2.toFixed(1), fill: 'none', stroke: '#8a8778', 'stroke-width': 1 }, parent);
  }
  var g = el('g', { transform: 'rotate(' + angle + ' ' + cx + ' ' + cy + ')' }, parent);
  var uw = 14;
  el('polygon', {
    points: (cx - uw) + ',' + (cy - 44) + ' ' + (cx + uw) + ',' + (cy - 44) + ' ' + (cx + uw) + ',' + (cy - 28) + ' ' + cx + ',' + (cy - 6) + ' ' + (cx - uw) + ',' + (cy - 28),
    fill: '#fff', stroke: INK, 'stroke-width': 1.6, 'stroke-linejoin': 'round'
  }, g);
  var lw = 17;
