...
<P>Piantiamo un pelo in ogni punto della superficie di una sfera e chiediamo tre cose: che ogni pelo sia aderente, cioè punti in una direzione tangente alla superficie; che passando da un punto a quelli vicini la direzione cambi con continuità, senza scatti; e che in nessun punto il pelo manchi o si rizzi. Pettinare la sfera vuol dire riuscire a disporre i peli così. Il teorema dice che non si può: comunque li si disponga, resta sempre almeno un punto in cui il pelo manca o si rizza.
|We plant a hair at every point of the surface of a sphere and ask three things: that every hair lie flat, that is, point in a direction tangent to the surface; that in passing from a point to its neighbours the direction change continuously, without jumps; and that at no point the hair be missing or stand up. To comb the sphere means to succeed in laying out the hairs in this way. The theorem says that it cannot be done: however they are laid out, there always remains at least one point where the hair is missing or stands up.</P>
...
|We do not give the proof, but the result can be sensed. Imagine combing the sphere along the meridians, all the hairs from the north pole towards the south pole: the combing succeeds everywhere except at the two poles, where the meridians converge and there is no direction to choose. Move the hairs as we like and the bad point moves with them, but it does not go away: with some ingenuity the two poles can be reduced to one, never to none.</P>

<P>Il legame con la nostra domanda passa per i numeri di modulo 1. In un’algebra sono i numeri per cui moltiplicare non cambia le grandezze, e in un’algebra di dimensione 3 sono i punti a distanza 1 dall’origine dello spazio: proprio la superficie di una sfera ordinaria. Faremo vedere che, se quell’algebra esistesse, il suo prodotto basterebbe a pettinare la sfera. Osserviamo intanto che la sfera è centrata nell’origine, e che su una sfera così essere tangente in un punto vuol dire essere perpendicolare a quel punto.
|The link with our question runs through the numbers of modulus 1. In an algebra these are the numbers by which multiplying does not change sizes, and in an algebra of dimension 3 they are the points at distance 1 from the origin of space: precisely the surface of an ordinary sphere. We shall show that, if that algebra existed, its multiplication would be enough to comb the sphere. Let us note meanwhile that the sphere is centred at the origin, and that on such a sphere being tangent at a point means being perpendicular to that point.</P>

<P>Il numero {{1}} sta sulla sfera: da {{|1|=|1\cdot 1|=|1|^2}} e {{|1|\neq 0}} segue {{|1|=1}}. In quel punto scegliamo una direzione tangente qualunque, cioè un vettore {{v}} non nullo perpendicolare a {{1}}.
|The number {{1}} lies on the sphere: from {{|1|=|1\cdot 1|=|1|^2}} and {{|1|\neq 0}} it follows that {{|1|=1}}. At that point let us choose any tangent direction, that is, a non-zero vector {{v}} perpendicular to {{1}}.</P>
...
<P>Nel punto {{u}} piantiamo allora il pelo {{vu}}, l’immagine di {{v}}. È aderente: {{v}} è perpendicolare a {{1}} e gli angoli si conservano, quindi {{vu}} è perpendicolare a {{u}}. Non manca mai: se {{vu}} fosse zero avrebbe modulo zero, ma {{|vu|=|v|\neq 0}}. E cambia con continuità, perché {{vu}} dipende da {{u}} in modo lineare. Al variare di {{u}} su tutta la sfera i peli ci sono tutti, aderenti e continui: la sfera è pettinata.
|At the point {{u}} let us then plant the hair {{vu}}, the image of {{v}}. It lies flat: {{v}} is perpendicular to {{1}} and angles are preserved, hence {{vu}} is perpendicular to {{u}}. It is never missing: if {{vu}} were zero it would have modulus zero, but {{|vu|=|v|\neq 0}}. And it changes continuously, because {{vu}} depends on {{u}} linearly. As {{u}} runs over the whole sphere the hairs are all there, lying flat and continuous: the sphere is combed.</P>

<P>Ma la sfera non si pettina. Dunque l’algebra a tre dimensioni non esiste: Hamilton stava cercando una cosa che non c’era.
|But the sphere cannot be combed. Hence the three-dimensional algebra does not exist: Hamilton was looking for something that was not there.</P>
