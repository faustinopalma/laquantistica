...
<P><strong>La divisione.</strong> Si deve poter dividere per qualunque numero diverso da zero. Risolvere {{ax=b}} vuol dire moltiplicare i due membri per l’inverso di {{a}}: se qualche numero diverso da zero non ha inverso, quell’equazione resta senza soluzione. Non è un caso di scuola. Nell’aritmetica dell’orologio a sei ore {{2\cdot 3}} fa {{0}} pur non essendo zero né {{2}} né {{3}}, e l’equazione {{2x=1}} non ha alcuna soluzione: {{2x}} vale sempre {{0}}, {{2}} o {{4}}, mai {{1}}. Chiedere che si possa sempre dividere è chiedere che questo non succeda.
|<strong>Division.</strong> It must be possible to divide by any number other than zero. Solving {{ax=b}} means multiplying both sides by the inverse of {{a}}: if some nonzero number has no inverse, that equation is left with no solution. This is not a textbook curiosity. In clock arithmetic with six hours {{2\cdot 3}} equals {{0}} although neither {{2}} nor {{3}} is zero, and the equation {{2x=1}} has no solution at all: {{2x}} is always {{0}}, {{2}} or {{4}}, never {{1}}. To require that division always be possible is to require that this not happen.</P>
...
<P>Queste quattro proprietà non sono un capriccio: sono quello che si adopera di continuo scrivendo una teoria fisica. Le grandezze si sommano quando più contributi si mettono insieme. Si moltiplicano quando una grandezza ne scala un’altra, o quando due operazioni si applicano una dopo l’altra. Si divide ogni volta che da una legge si ricava un’incognita: risolvere per {{x}} è dividere. E il modulo serve perché i conti vanno confrontati con numeri letti su uno strumento, che sono reali e hanno una grandezza; se il modulo non si moltiplicasse insieme ai numeri, dopo un calcolo lungo non sapremmo stimare nemmeno l’ordine di grandezza del risultato.
|These four properties are no whim: they are what one uses constantly in writing a physical theory. Quantities are added when several contributions are put together. They are multiplied when one quantity scales another, or when two operations are applied one after the other. One divides whenever an unknown is drawn out of a law: solving for {{x}} is dividing. And the modulus is needed because calculations have to be compared with numbers read off an instrument, which are real and have a size; if the modulus did not multiply along with the numbers, after a long calculation we could not even estimate the order of magnitude of the result.</P>
...
<P>Su quella superficie, in ogni punto, possiamo immaginare le direzioni tangenti. Nel punto che corrisponde al numero 1 scegliamone una. Moltiplicare per un numero {{u}} di modulo 1 porta il punto 1 nel punto {{u}}, e trascina con sé quella direzione.
|On that surface, at every point, we can picture the tangent directions. At the point corresponding to the number 1 let us choose one of them. Multiplying by a number {{u}} of modulus 1 carries the point 1 to the point {{u}}, and drags that direction along with it.</P>

<P>Che l’operazione funzioni lo garantiscono le proprietà elencate. Moltiplicare per {{u}} non cambia i moduli, perché {{|xu|=|x|\,|u|=|x|}}: la superficie va dunque in sé stessa, e le direzioni tangenti restano tangenti. L’operazione si può invertire, basta dividere per {{u}}, e perciò non schiaccia mai una direzione a zero. Ed è continua, perché il prodotto si distribuisce sulla somma. Facendo variare {{u}} su tutta la superficie, la direzione scelta nel punto 1 si trasporta dappertutto con continuità, senza annullarsi mai.
|That the operation works is guaranteed by the properties we listed. Multiplying by {{u}} does not change moduli, since {{|xu|=|x|\,|u|=|x|}}: the surface therefore maps onto itself, and tangent directions stay tangent. The operation can be inverted, one need only divide by {{u}}, and so it never crushes a direction to zero. And it is continuous, because the product distributes over addition. Letting {{u}} range over the whole surface, the direction chosen at the point 1 is carried everywhere continuously, without ever vanishing.</P>

<P>Una direzione tangente in ogni punto, che varia con continuità e non si annulla mai: è quello che si ottiene piantando un pelo in ogni punto della superficie e pettinandoli tutti in modo che restino aderenti. Se l’algebra a tre dimensioni esiste, allora la sfera si lascia pettinare.
|A tangent direction at every point, varying continuously and never vanishing: that is what one obtains by planting a hair at every point of the surface and combing them all so that they lie flat. If the three-dimensional algebra exists, then the sphere can be combed.</P>

<P>Il teorema della palla pelosa dice che non si lascia pettinare. Su una sfera, per quanto ci si provi, resta sempre almeno un punto in cui il pelo si rizza oppure manca. Sulla circonferenza invece si riesce, pettinando tutti i peli nello stesso verso di percorrenza: e infatti l’algebra di dimensione 2 esiste, ed è quella dei numeri complessi. Sulla sfera no: l’algebra di dimensione 3 non può esistere. Hamilton stava cercando una cosa che non c’era.
|The hairy ball theorem says that it cannot be combed. On a sphere, however hard one tries, there always remains at least one point where a hair stands up or is missing. On a circle it can be done, by combing all the hairs the same way round: and indeed the algebra of dimension 2 exists, and it is that of the complex numbers. On the sphere it cannot: the algebra of dimension 3 cannot exist. Hamilton was looking for something that was not there.</P>
...
<P>Con questa convenzione l’elenco si legge bene. Le sole sfere che si lasciano pettinare per intero — cioè sulle quali si può disporre con continuità non una sola direzione tangente, ma un intero sistema di direzioni indipendenti — sono quelle di dimensione 0, 1, 3 e 7, e sono fatte esattamente dai numeri di modulo 1 delle quattro algebre. Conviene guardarle una per una.
|With this convention the list reads clearly. The only spheres that can be combed entirely — that is, on which one can lay out continuously not just a single tangent direction, but a whole system of independent directions — are those of dimension 0, 1, 3 and 7, and they consist precisely of the numbers of modulus 1 of the four algebras. It is worth looking at them one at a time.</P>
...
<P><strong>Dimensione 3.</strong> I quaternioni di modulo 1 formano una sfera che vive nello spazio a quattro dimensioni: per dire dove ci troviamo servono tre numeri, come su una sfera ordinaria ne servono due. Disegnarla non possiamo, ma le formule la trattano come le altre, e dicono che si lascia pettinare per intero: in ogni punto si possono disporre tre direzioni indipendenti.
|<strong>Dimension 3.</strong> The quaternions of modulus 1 form a sphere living in four-dimensional space: to say where we are, three numbers are needed, just as two are needed on an ordinary sphere. We cannot draw it, but the formulas treat it like the others, and they say it can be combed entirely: at every point three independent directions can be laid out.</P>
