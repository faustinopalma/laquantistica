...
<meta name="description" content="Perché usiamo i numeri complessi: che cosa deve avere un'algebra, le quattro sole possibilità, perché manca quella a tre dimensioni, ampiezza e fase delle onde, il campo di Riemann-Silberstein, il problema agli autovalori e il ritorno alle soluzioni reali.">
...
<P>Queste sono le proprietà che chiediamo. Da qui partirà tutto il resto.
|These are the properties we ask for. Everything else will start from here.</P>
...
|Useful does not mean compulsory. Nothing assures us that nature lets itself be described in this way. On a clock with a prime number of hours, for instance, addition, multiplication and division all work — there every number other than zero has its own inverse — and there are no real numbers at all: a world made that way cannot be ruled out. These properties are not imposed by experience, we choose them, and the conclusion that follows is worth as much as the premises are.</P>
...
<P>Che ciascuna delle quattro abbia davvero le proprietà richieste è un fatto da verificare, e per gli ottonioni non è scontato. Il punto delicato è il modulo. Chiedere {{|xy|=|x|\,|y|}} per numeri di {{n}} componenti significa chiedere un’identità algebrica: che il prodotto di due somme di {{n}} quadrati sia ancora una somma di {{n}} quadrati, e che quei quadrati siano scritti in modo bilineare nelle componenti dei due fattori. Per {{n=2}} l’identità si verifica in due righe; per {{n=4}} è l’identità dei quattro quadrati, nota a Euler prima che Hamilton trovasse i quaternioni; per {{n=8}} è quella degli otto quadrati, trovata da Degen e ritrovata da Graves. Funziona, ma non c’è nulla di inevitabile: il teorema di Hurwitz dice che per nessun altro {{n}} un’identità di questa forma esiste.
|That each of the four really has the required properties is something to be checked, and for the octonions it is not obvious. The delicate point is the modulus. To ask {{|xy|=|x|\,|y|}} for numbers of {{n}} components is to ask for an algebraic identity: that the product of two sums of {{n}} squares be again a sum of {{n}} squares, and that those squares be written bilinearly in the components of the two factors. For {{n=2}} the identity is checked in two lines; for {{n=4}} it is the four-square identity, known to Euler before Hamilton found the quaternions; for {{n=8}} it is the eight-square identity, found by Degen and rediscovered by Graves. It works, but there is nothing inevitable about it: Hurwitz’s theorem says that for no other {{n}} does an identity of this form exist.</P>

<P>Torniamo alla divisione, che negli ottonioni — dove il prodotto non è associativo — andrebbe maneggiata con cura. Non occorre verificarla a parte: se il modulo si moltiplica, allora {{xy=0}} obbliga {{|x|\,|y|=0}}, dunque uno dei due fattori è nullo. Moltiplicare per un numero diverso da zero è allora un’operazione lineare che non manda nulla in zero, e in dimensione finita questo basta perché sia invertibile: le equazioni {{ax=b}} e {{xa=b}} hanno sempre una e una sola soluzione. La divisione segue dal modulo.
|Let us return to division, which among the octonions — where multiplication is not associative — would have to be handled with care. There is no need to check it separately: if the modulus multiplies, then {{xy=0}} forces {{|x|\,|y|=0}}, so one of the two factors is zero. Multiplying by a number other than zero is then a linear operation that sends nothing to zero, and in finite dimension this is enough for it to be invertible: the equations {{ax=b}} and {{xa=b}} always have one and only one solution. Division follows from the modulus.</P>

<P>Vediamo ora che cosa significa ciascuna perdita, perché è su queste che decideremo.
|Let us now see what each loss means, because it is on these that we shall decide.</P>
...
<P>In cambio della perdita c’è un guadagno, e conviene dirlo subito perché più avanti sarà decisivo: nel campo complesso ogni equazione polinomiale non costante ha almeno una soluzione. Nei reali non è così — {{x^2+1=0}} non ne ha — ed è proprio per risolverla che {{i}} è nato. Questa proprietà si chiama <em>chiusura algebrica</em>, e i complessi sono la più piccola estensione dei reali che la possiede. Nei quaternioni e negli ottonioni la questione si complica: un teorema di Eilenberg e Niven garantisce una radice per i polinomi scritti con i coefficienti tutti da una parte, come {{aq^2+bq+c}}; ma senza commutatività i coefficienti possono anche infilarsi tra le potenze dell’incognita, e per espressioni di quel tipo la garanzia cade — l’equazione {{qi-iq=1}}, per esempio, non ha alcuna soluzione, perché il primo membro non ha mai parte reale. La chiusura piena, senza distinguo, ce l’hanno soltanto i complessi.
|In exchange for the loss there is a gain, and it is worth saying at once because further on it will be decisive: over the complex field every nonconstant polynomial equation has at least one solution. Over the reals this is not so — {{x^2+1=0}} has none — and it was precisely in order to solve it that {{i}} was born. This property is called <em>algebraic closure</em>, and the complex numbers are the smallest extension of the reals that possesses it. Among the quaternions and the octonions the matter becomes more complicated: a theorem of Eilenberg and Niven guarantees a root for polynomials written with all the coefficients on one side, such as {{aq^2+bq+c}}; but without commutativity the coefficients can also slip in between the powers of the unknown, and for expressions of that kind the guarantee fails — the equation {{qi-iq=1}}, for instance, has no solution at all, because the left-hand side never has a real part. Full closure, with no qualifications, belongs to the complex numbers alone.</P>
...
<P>Tirando le somme: i numeri complessi sono l’ultima algebra in cui il prodotto resta commutativo e associativo, l’unica proprietà che vi perdiamo è l’ordinamento, di cui non abbiamo bisogno, e in cambio guadagniamo la chiusura algebrica, che nessuna delle altre possiede per intero.
|To sum up: the complex numbers are the last algebra in which multiplication remains commutative and associative, the only property we lose there is the ordering, which we do not need, and in exchange we gain algebraic closure, which none of the others possesses in full.</P>
...
|Let us first say what combing means. We plant a hair at every point of the surface and ask three things: that every hair lie flat, that is, point in a direction tangent to the surface; that in passing from a point to its neighbours the direction change continuously, without jumps; and that at no point the hair be missing or stand up. To comb the surface means to succeed in laying out the hairs in this way.</P>

<P>Ci serve anche un’osservazione sulla tangenza. La nostra sfera è centrata nell’origine, e su una sfera così il piano tangente in un punto {{p}} è fatto esattamente dei vettori perpendicolari a {{p}}: dire <em>tangente in {{p}}</em> e dire <em>perpendicolare a {{p}}</em> è la stessa cosa.
|We also need an observation about tangency. Our sphere is centred at the origin, and on such a sphere the tangent plane at a point {{p}} consists exactly of the vectors perpendicular to {{p}}: saying <em>tangent at {{p}}</em> and saying <em>perpendicular to {{p}}</em> is the same thing.</P>

<P>Ora facciamo vedere che se l’algebra a tre dimensioni esiste, la sfera si pettina.
|Let us now show that if the three-dimensional algebra exists, the sphere can be combed.</P>

<P>Intanto il numero {{1}} sta sulla sfera: da {{|1|=|1\cdot 1|=|1|^2}} e {{|1|\neq 0}} segue {{|1|=1}}. In quel punto scegliamo una direzione tangente qualunque — cioè un vettore {{v}} non nullo perpendicolare a {{1}}.
|To begin with, the number {{1}} lies on the sphere: from {{|1|=|1\cdot 1|=|1|^2}} and {{|1|\neq 0}} it follows that {{|1|=1}}. At that point let us choose any tangent direction — that is, a non-zero vector {{v}} perpendicular to {{1}}.</P>

<P>Prendiamo poi un punto qualunque della sfera, cioè un numero {{u}} di modulo 1, e consideriamo l’operazione che a ogni {{x}} associa {{xu}}. È lineare: le somme si spezzano perché il prodotto si distribuisce, e i fattori reali escono perché lo abbiamo chiesto tra le proprietà. E non cambia i moduli, perché {{|xu|=|x|\,|u|=|x|}}. Avendo preso il modulo uguale alla distanza dall’origine, l’operazione conserva tutte le distanze: è una trasformazione rigida dello spazio, e le trasformazioni rigide conservano anche gli angoli. Porta la sfera in sé stessa e porta il punto {{1}} nel punto {{u}}.
|Now take any point of the sphere, that is, a number {{u}} of modulus 1, and consider the operation that to every {{x}} assigns {{xu}}. It is linear: sums split up because multiplication distributes, and real factors come out because we asked for that among the properties. And it does not change the moduli, because {{|xu|=|x|\,|u|=|x|}}. Having taken the modulus equal to the distance from the origin, the operation preserves all distances: it is a rigid transformation of space, and rigid transformations preserve angles as well. It carries the sphere into itself and carries the point {{1}} to the point {{u}}.</P>

<P>Nel punto {{u}} piantiamo allora il pelo {{vu}}, l’immagine di {{v}}. Verifichiamo le tre richieste una per una. È aderente: {{v}} è perpendicolare a {{1}}, l’operazione conserva gli angoli, quindi {{vu}} è perpendicolare a {{u}} — e perpendicolare a {{u}} vuol dire tangente in {{u}}. Non manca mai: se {{vu}} fosse zero avrebbe modulo zero, ma {{|vu|=|v|\neq 0}}. E cambia con continuità, perché {{vu}} dipende da {{u}} in modo lineare. Al variare di {{u}} su tutta la sfera abbiamo un pelo in ogni punto, aderente, continuo e mai mancante: la sfera è pettinata.
|At the point {{u}} let us then plant the hair {{vu}}, the image of {{v}}. Let us check the three requirements one by one. It lies flat: {{v}} is perpendicular to {{1}}, the operation preserves angles, hence {{vu}} is perpendicular to {{u}} — and perpendicular to {{u}} means tangent at {{u}}. It is never missing: if {{vu}} were zero it would have modulus zero, but {{|vu|=|v|\neq 0}}. And it changes continuously, because {{vu}} depends on {{u}} linearly. As {{u}} runs over the whole sphere we have a hair at every point, lying flat, continuous and never missing: the sphere is combed.</P>

<P>Il teorema della palla pelosa dice che la sfera non si lascia pettinare: comunque si dispongano i peli, resta sempre almeno un punto in cui il pelo manca o si rizza. La dimostrazione non la diamo, ma il risultato si lascia intuire. Immaginiamo di pettinare la sfera lungo i meridiani, tutti i peli dal polo nord verso il polo sud: la pettinatura riesce ovunque, tranne che ai due poli, dove i meridiani convergono e la direzione da scegliere non c’è. Spostiamo i peli come vogliamo e il punto cattivo si sposta con loro, ma non se ne va: con un po’ di astuzia i due poli si riducono a uno solo, mai a nessuno.
|The hairy ball theorem says that the sphere cannot be combed: however the hairs are laid out, there always remains at least one point where the hair is missing or stands up. We do not give the proof, but the result can be sensed. Imagine combing the sphere along the meridians, all the hairs from the north pole towards the south pole: the combing succeeds everywhere except at the two poles, where the meridians converge and there is no direction to choose. Move the hairs as we like and the bad point moves with them, but it does not go away: with some ingenuity the two poles can be reduced to one, never to none.</P>

<P>Dunque: se l’algebra a tre dimensioni ci fosse, la sfera si pettinerebbe; la sfera non si pettina; quell’algebra non esiste. Hamilton stava cercando una cosa che non c’era.
|Hence: if the three-dimensional algebra existed, the sphere could be combed; the sphere cannot be combed; that algebra does not exist. Hamilton was looking for something that was not there.</P>

<P>Sulla circonferenza invece i peli si dispongono, mandandoli tutti nello stesso verso di percorrenza. Attenzione però: questo non dimostra che l’algebra di dimensione 2 esista. L’implicazione va in un senso solo, e quando la superficie si pettina non conclude nulla: dice soltanto che in dimensione 2 questo ostacolo non c’è. Che l’algebra ci sia lo sappiamo perché i numeri complessi li abbiamo davanti, non perché il ragionamento lo imponga.
|On the circle, by contrast, the hairs can be laid out, all of them sent the same way round. Note, however: this does not prove that the algebra of dimension 2 exists. The implication runs one way only, and when the surface can be combed it concludes nothing: it says merely that in dimension 2 this obstacle is absent. That the algebra is there we know because the complex numbers are before our eyes, not because the argument compels it.</P>

<P>Va tolto di mezzo, infine, un equivoco di parole. In matematica la sfera è la sola superficie, e la sua dimensione è quella della superficie, non dello spazio che la contiene: per dire dove ci troviamo su una sfera bastano due numeri, come la latitudine e la longitudine, quindi la sfera ordinaria ha dimensione 2 pur vivendo nello spazio a tre dimensioni. Quando diciamo che manca l’algebra di dimensione 3, il 3 è la dimensione dell’algebra, non quella della sfera.
|One verbal misunderstanding must finally be cleared away. In mathematics the sphere is the surface alone, and its dimension is that of the surface, not of the space containing it: to say where we are on a sphere two numbers suffice, such as latitude and longitude, so the ordinary sphere has dimension 2 while living in three-dimensional space. When we say that the algebra of dimension 3 is missing, the 3 is the dimension of the algebra, not that of the sphere.</P>

<P>Devo però dire fin dove arrivo io. Di questa sezione abbiamo dimostrato una cosa sola: che se l’algebra esiste, la sua sfera si pettina. L’altro pezzo — che la sfera ordinaria non si pettina — lo abbiamo preso per acquisito, e lo stesso vale per il teorema di Hurwitz, da cui viene la tabella delle quattro algebre. Dietro a entrambi c’è una matematica vasta: topologia algebrica, teoria dei fibrati, cose che si studiano per anni e che io non conosco. Mi limito a riportare le conclusioni di chi le ha costruite: le strade percorribili sono quattro, e per la scelta che dobbiamo fare questo basta. Spero di poterle studiare, un giorno.
|I must say, though, how far I myself get. Of this section we have proved one thing only: that if the algebra exists, its sphere can be combed. The other piece — that the ordinary sphere cannot be combed — we have taken as given, and the same holds for Hurwitz’s theorem, from which the table of the four algebras comes. Behind both lies a vast body of mathematics: algebraic topology, the theory of fibre bundles, things one studies for years and that I do not know. I merely report the conclusions of those who built them: the practicable roads are four, and for the choice we have to make this is enough. I hope to be able to study them one day.</P>
...
<P>Il risultato sulle quattro algebre è il teorema di Hurwitz (1898), che si può enunciare anche come classificazione delle identità fra somme di quadrati: esistono per {{n=1,2,4,8}} e per nessun altro {{n}}. L’identità dei quattro quadrati è di Euler (1748); quella degli otto è di C. F. Degen (1818) e fu ritrovata indipendentemente da J. T. Graves (1843) e da A. Cayley (1845), insieme agli ottonioni. Il teorema della palla pelosa è di L. E. J. Brouwer (1912). Per il campo elettromagnetico complesso abbiamo seguito la forma {{\mathbf F=\mathbf E+ic\mathbf B}}, introdotta nella teoria elettromagnetica da H. M. Weber e L. Silberstein e oggi nota come vettore di Riemann-Silberstein; una trattazione moderna si trova in I. Bialynicki-Birula, <em>Photon wave function</em>, <em>Progress in Optics</em> 36 (1996), pp. 245–294.
|The result on the four algebras is Hurwitz’s theorem (1898), which can also be stated as a classification of the identities between sums of squares: they exist for {{n=1,2,4,8}} and for no other {{n}}. The four-square identity is due to Euler (1748); the eight-square identity to C. F. Degen (1818), rediscovered independently by J. T. Graves (1843) and by A. Cayley (1845), together with the octonions. The hairy ball theorem is due to L. E. J. Brouwer (1912). For the complex electromagnetic field we followed the form {{\mathbf F=\mathbf E+ic\mathbf B}}, introduced into electromagnetic theory by H. M. Weber and L. Silberstein and now known as the Riemann-Silberstein vector; a modern treatment can be found in I. Bialynicki-Birula, <em>Photon wave function</em>, <em>Progress in Optics</em> 36 (1996), pp. 245–294.</P>
'''
